...
#     robot_obj = mc.MeshFileGeometry(joinpath(@__DIR__,"quadrotor.obj"))
    robot_obj = mc.MeshFileGeometry(joinpath(@__DIR__,"quadrotor_scaled.obj"))    
    mc.setobject!(vis[:vic], robot_obj, mc.MeshPhongMaterial(color = mc.RGBA(0.0,0.0,0.0,1.0)))

    vis_traj!(vis, :traj, Xref[1:85]; R = 0.01, color = mc.RGBA(1.0, 0.0, 0.0, 0.5))
